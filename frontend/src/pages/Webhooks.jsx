            ...
            <div className="p-4 lg:p-8 max-w-6xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-text-secondary mt-1 text-sm">Configure endpoints to receive real-time events from the system.</p>
                    </div>
                    <button
                        ...
                        <table className="w-full text-left text-sm text-text-secondary">
                            <thead className="bg-background border-b border-border text-xs uppercase font-medium">
                                <tr>
                                    ...
                                    <th className="px-6 py-4">Events Subscribed</th>
                                ...
                                {loading ? (
                                    <tr>
                                        <td colSpan="4" className="px-6 py-12 text-center">
                                            ...
                                            <span className="text-text-secondary">Loading endpoints...</span>
                                        </td>
                                    </tr>
                                ) : endpoints.length === 0 ? (
                                    <tr>
                ...
                <div className="bg-primary/5 border border-primary/20 rounded-2xl p-5 flex gap-4 text-sm text-text-primary">
                    ...
                    <div>
                        <h4 className="font-bold mb-1">Testing Webhooks</h4>
                        <p className="text-text-secondary leading-relaxed">
                            Need an endpoint to test? We recommend using a free service like <a href="https://webhook.site" target="_blank" rel="noreferrer" className="text-primary hover:underline font-medium">Webhook.site</a>. 
                            It gives you a unique URL to monitor incoming HTTP requests instantly without setting up your own server.
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-surface rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 border border-border">
                        <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-background">
                            <h3 className="font-bold text-lg text-text-primary">Add Webhook Endpoint</h3>
                        ...
                        <form onSubmit={handleAddSubmit} className="p-6 space-y-5">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-text-secondary mb-1.5">Payload URL</label>
                                <input 
                                    type="url" 
                                    required
                            ...
                            <div>
                                ...
                                {formData.events.length === 0 && (
                                    <p className="text-xs text-danger mt-1">Please select at least one event.</p>
                                )}
                            </div>
                            <div className="flex items-center gap-2 pt-2">
                                <label className="relative inline-flex items-center cursor-pointer">
                                    ...
                                    <span className="ml-3 text-sm font-medium text-text-primary">Endpoint Active</span>
                                </label>
                            </div>
                            <div className="flex gap-3 pt-4 border-t border-border">
                                ...
                                >
                                    {actionLoading === 'add' ? <Loader2 size={18} className="animate-spin" /> : 'Create Webhook'}
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div className="bg-surface rounded-2xl shadow-xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[80vh] border border-border">
                        <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-background shrink-0">
                            <h3 className="font-bold text-lg text-text-primary flex items-center gap-2">
                                <Activity className="w-5 h-5 text-primary" /> Delivery Logs
                            </h3>
